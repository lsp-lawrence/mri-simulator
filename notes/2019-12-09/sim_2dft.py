...
for j,x in zip(range(grid_dim),x_vals):
    for i,y in zip(range(grid_dim),y_vals):
        #num_ems_here = int(np.exp(-(x**2)/(2*sigma**2))*max_ems_grid_point)
        num_ems_here = max_ems_grid_point
